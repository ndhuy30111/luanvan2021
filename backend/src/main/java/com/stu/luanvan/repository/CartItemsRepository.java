...
public interface CartItemsRepository extends JpaRepository<CartItemsModel, Integer> {
    ...
    @Query(value = "SELECT new com.stu.luanvan.response.CartItemsResponse(p.name,p.price,f.url,ci.quantity,ci.color,ci.size)  FROM `cart_items` ci " +
    ...
    List<CartItemsResponse> findByUserAndColorName(@Param("id") Integer id);
}
